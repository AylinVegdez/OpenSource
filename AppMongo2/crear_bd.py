...
def regresar_imagen_producto(idProducto):
    for prod in img_productos:
        if prod[1]==idProducto:
            return prod[2]
    return "productoDefault.webp"
    ...
    for prod in productos:
        ...
        for i in range(len(atributos_productos)):
            ...
            if atributos_productos[i] == "idCategoria":
                ...
                dicc_producto["idCategoria"]=dicc_categoria
                dicc_producto["codigoBarras"]=dicc_categoria["nombreCategoriaProducto"]+"-"+str(prod[0]) #categoria y id del producto
                continue
            #print(atributos_productos[i])
            if atributos_productos[i]=="productoCosto":
                dicc_producto["productoCosto"] = float(prod[i])
                dicc_producto["listaproductoCosto"]=[
                    {"costo":dicc_producto["productoCosto"],"fecha":"2022-10-31","vigente":True}
                ]
                continue
            dicc_producto[atributos_productos[i]]=prod[i]
        #campos adicionales
        dicc_producto["productoImagen"]=regresar_imagen_producto(prod[0])
        dicc_producto["productoUtilidad"]=[
            {"id":1,"cantidad":3,"utilidad":15},
            {"id": 2, "cantidad": 6, "utilidad": 8},
            {"id": 3, "cantidad": 10, "utilidad": 3},

        ]
        #print(dicc_producto)
        objPyMongo.insertar('productos',dicc_producto)
    objPyMongo.desconectar_mongodb()
...
def cargar_tablas():
    ...
    tabla_pedido={
        ...
        "fecha": "2022-10-31",
        "total": 0.0,
        "descuento": 0.0, #descuento total del pedido
        "pagado":True,
        "estado":1,#1- Embalaje, 2-Recoleccion,3- ListoEnviar,4- EnCamino
        "productosPedidos": [
                {
                    "idProductos": 50,
                    "nombreProductos": "Producto ejemplo",
                    "cantidad": 1,
                    "utilidad":0.0,
                    "descuento": 0.0,
                    "precioFinal": 0.0,
                    "subTotal": 0.0, #calculo del precio final *cantidad
                    "imagen": "prueba.jpg"
                }
            ]
        }
    ...
    tabla_ofertas={
        "id":1,
        "descuento":{"id":1,"cantidad":10,"descuento":15},
        "fecha":"2022-10-31",
        "productos":[
            {"id":50},
            {"id": 1050}],
        "activo":True,
        "titulo":"15% de descuento al llevar 10 piezas",
    }
    tabla_clientes={
        "idCliente":1,
        ...
        "direccion":[
                {
                    ...
                    "codigoPostal":"06000",
                    "referencias":[
                        {"referencia":"frente al CBTis 12"},
                        {"referencia": "Entre las calles Florencia y Margaritas"},
                    ],
                    "activa":True
                }
            ]
